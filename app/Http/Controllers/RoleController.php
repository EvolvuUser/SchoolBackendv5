...
class RoleController extends Controller
{
    ...
    public function update(Request $request, $role_id)
    {
        $validatedData = $request->validate([
            'rolename' => [
                ...
                'string',
                'max:50',
                Rule::unique('role_master', 'name')->ignore($role_id, 'role_id'),
            ],
            'role_id' => [
                'required',
                'string',
                'max:1',
                Rule::unique('role_master', 'role_id')->ignore($role_id, 'role_id'),
            ],
        ]);
    ...
    public function navLeafMenus(Request $request)
    {
        ...
        $leafMenus = $assignedMenus
            ->whereNotIn('menu_id', $parentIds)
            ->sortBy('sequence')
            ->values()
            ->map(function ($menu) {
                return [
